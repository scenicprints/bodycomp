# ═══════════════════════════════════════════════════════════════════════
#  BodyComp — publish a new release
#
#  Usage:
#     .\publish.ps1                       (prompts for version + notes)
#     .\publish.ps1 -Version 1.0.1 -Notes "Fixed the trend chart"
#
#  What it does:
...
# --- get target version ---
if (-not $Version) {
    $Version = Read-Host "New version number (e.g. 1.0.1) [current name: $currentName]"
}
if ($Version -notmatch '^\d+\.\d+\.\d+$') {
    Write-Error "Version must look like 1.0.1"; exit 1
}
...
Write-Host "Watch progress:  https://github.com/scenicprints/bodycomp/actions" -ForegroundColor Cyan
