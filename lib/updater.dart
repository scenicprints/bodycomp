...
// IN-APP OTA UPDATER
//
// Checks the latest GitHub Release for this repo, compares its version to
// the installed one, and (if newer) downloads + installs the new APK.
//
// The repo is PUBLIC, so the releases API needs no token. GitHub allows
// 60 unauthenticated requests/hour per IP — plenty for a personal app.
// ═══════════════════════════════════════════════════════════════════════

const String kRepoOwner = 'scenicprints';
const String kRepoName = 'bodycomp';

class ReleaseInfo {
  final String version; // e.g. "1.0.1" (the tag, minus any leading "v")
      ...
      OtaUpdate()
        ...
        switch (event.status) {
          ...
          case OtaStatus.INSTALLATION_DONE:
            // Android's package installer takes over from here.
            break;
          ...
          case OtaStatus.PERMISSION_NOT_GRANTED_ERROR:
            setState(() {
              _s = _State.error;
              _msg = 'Allow "install unknown apps" for BodyComp, then retry.';
            });
            break;
    ...
    switch (_s) {
      ...
      case _State.error:
        return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(_msg,
              style:
                  const TextStyle(fontSize: 13, color: Color(0xFFCC8855))),
